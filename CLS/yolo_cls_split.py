...
def split_dataset(
    source_dir, output_dir, train_ratio=0.8, val_ratio=0.1, test_ratio=0.1
):
    ...
    for cls in classes:
        ...
        print(
            f"Hoàn thành class {cls}: Train({len(train_files)}), Val({len(val_files)}), Test({len(test_files)})"
        )


# Cấu hình đường dẫn của bạn ở đây
source_data = (
    r"D:\9.ATC_AI_Core\SVSC2026\DATASET\Animal3"  # Thư mục chứa 3 folder cat, dog, fox
)
output_data = os.path.join(
    os.path.dirname(source_data), "Animal3_yolo_dataset"
)  # Thư mục đích sẽ dùng để train
